...
<%! 
	...
	public int queryCoordinates(final String strAppId, final String strStartDate, final String strEndDate, final String strStartHour,
		...
		int status = select(null,
				"SELECT `latitude`, `longitude` FROM (SELECT `table_name` FROM `tracker`.`app_list` WHERE `app_id` =?) AS table WHERE `create_date` BETWEEN ? AND ? AND HOUR(`create_date`) BETWEEN ? AND ?",
