...
<%! 
	...
	public int queryCoordinates(final String strAppId, final String strStartDate, final String strEndDate, final String strStartHour,
		...
		int status = select(conn,
				"SELECT t.`latitude`, t.`longitude` FROM (SELECT `table_name` FROM `tracker`.`app_list` WHERE `app_id` =?) AS t WHERE t.`create_date` BETWEEN ? AND ? AND HOUR(t.`create_date`) BETWEEN ? AND ?",
