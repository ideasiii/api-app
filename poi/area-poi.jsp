	...
	private JSONObject processRequest(HttpServletRequest request) throws ServletException, IOException
{
	...
	public int queryAreaPoiArray(final String strAppId,final String strCategory, final String strStartDate, final String strEndDate, final String strArea, final String strPoiList, final JSONArray out) {
		...
		int status = select(null, "SELECT `poi`, `category`, `count` FROM `app_user_pre_area_poi` WHERE `app_id`=? AND `start_date`=? AND `end_date`=? AND `area`=? AND `category`=? AND `poi` IN " + strPoiList + " ORDER BY `count` DESC",
	...
	public int queryAllCatePoiArray(final String strAppId, final String strStartDate, final String strEndDate, final String strArea, final String strPoiList, final JSONArray out) {
		...
		int status = select(null, "SELECT `poi`, `category`, `count` FROM `app_user_pre_area_poi` WHERE `app_id`=? AND `start_date`=? AND `end_date`=? AND `area`=? AND `poi` IN " + strPoiList + " ORDER BY `count` DESC",
